...
class MalthusianUniform(BaseEnvironment):
    """
    ...
    name = "malthusian_uniform/simple_wood_and_stone"
    ...
    def __init__(
        ...
        *base_env_args,
        ...
        starting_wood_coverage=0.03,
        wood_regen_halfwidth=0,
        wood_regen_weight=0.02,
        wood_max_health=3,
        starting_stone_coverage=0.03,
        stone_regen_halfwidth=0,
        stone_regen_weight=0.02,
        stone_max_health=3,
        ...
        for nation in self.world.planner.state['nations']:
            if self.planner_reward_type == "coin_eq_times_productivity":
                ...
                curr_optimization_metric[
                    ...
                    utilities=np.array(
                        [curr_optimization_metric[agent.idx] for agent in self.world.agents if agent.state['nation'] == nation]
                    ),
                )
            else:
    ...
    def generate_observations(self):
        ...
        if 'p' not in obs:
            ...
            for nation in self.world.planner.state['nations']:
                obs['p'][nation] = dict()
        for nation in self.world.planner.state['nations']:
            obs[self.world.planner.idx][nation] = {
                "inventory-" + k: v * self.inv_scale
                for k, v in self.world.planner.inventory.items() if nation in k
            }
...
@scenario_registry.add
class MalthusianMultiZone(MalthusianUniform):
    """
    ...
    name = "malthusian_multi_zone/simple_wood_and_stone"
...
@scenario_registry.add
class MalthusianQuadrant(MalthusianUniform):
    """
    ...
    name = "malthusian_quadrant/simple_wood_and_stone"
    required_entities = MalthusianUniform.required_entities + ["Water"]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        width = self.world_size[1]
        height = self.world_size[0]
        self.world.zones = np.array([
