...
class MalthusianPeriodicBracketTax(BaseComponent):
    ...
    name = "MalthusianPeriodicBracketTax"
    ...
    def __init__(
        ...
        **base_component_kwargs
        ...
        # self.world.planner.state['nations'] = nations
        # self.world.planner.state['nations']_to_idx = dict((n, i) for i, n in enumerate(self.world.planner.state['nations']))
        # self.idx_to_nations = dict((i, n) for i, n in enumerate(self.world.planner.state['nations']))
        ...
        for nation in self.world.planner.state['nations']:
            for i, bracket in enumerate(self.bracket_cutoffs):
                planner_action = self.world.planner.get_component_action(
                    self.name, "TaxIndexBracket_{}_{:03d}".format(nation, int(bracket))
                )
                if planner_action == 0:
                    pass
                elif planner_action <= self.n_disc_rates:
                    self.curr_rate_indices[nation][i] = int(planner_action - 1)
                else:
                    raise ValueError
    ...
    def marginal_rate(self, income, nation):
    ...
    def get_additional_state_fields(self, agent_cls_name):
        ...
        if agent_cls_name == 'BasicPlanner':
            return {'tax_cycle_pos': self.tax_cycle_pos,
                    'period': self.period,
                    'taxes': self.taxes}
        return {}
    ...
    def generate_observations(self):
        ...
        for agent in self.world.agents:
            ...
            curr_marginal_rate = self.marginal_rate(agent.total_endowment("Coin") - self.last_coin[i],
                                                    agent.state['nation'])
